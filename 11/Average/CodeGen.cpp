...
void compileTerm(){
    ...
    if(Token[tno].data == "("){
        ...
        tno++; //past the )
        tno++; //past the </term>
            cout<<"exiting: term "<<Token[tno-1].data<<endl;
    ...
    if(Token[tno].type == "keywordConstant"){
    ...
    if(Token[tno].data == "term"){
        ...
        while(isop()){
            ...
            if(op=="&lt;")
                ss+="lt\n";
            if(op=="*")
                ss+="call Math.multipy 2\n";
            if(op=="/")
                ss+="call Math.divide 2\n";
        }
    }
    cout<<"exiting: Expression "<<Token[tno].data<<endl;

    tno++; //past the </expression>
}

void compileParameterList(){
    cout<<"entered ParameterList\n";
    ...
    cout<<"exiting: ParameterList"<<Token[tno].data<<endl;
...
void compileVarDec(){
    ...
    tno++; //past the ;
    cout<<"exiting: VarDec "<<Token[tno].data<<endl;
    tno++;
}
...
void compileLetStatement(){
    ...
    cout<<"exiting: LetStatement "<<Token[tno-1].data<<endl;
...
void compileIfStatement(){
    ...
    cout<<"exiting: ifStatement "<<Token[tno].data<<endl;
...
void compileWhileStatement(){
    ...
    ss+="label " + currentClassName + "." + to_string(tlabelnum + 1) + "\n";
    tno++; //past the }
    cout<<"exiting: whileStatement "<<Token[tno].data<<endl;
    tno++; //past the </while>

}

int compileExpressionList(){
    cout<<"entered ExpressionList\n";
    int np = 0;
    ...
    cout<<"exiting: ExpressionList "<<Token[tno].data<<endl;
...
void compileDoStatement(){
    ...
    cout<<"exiting: Do "<<Token[tno-1].data<<endl;

}

void compileReturnStatement(){
    cout<<"entering ReturnStatement\n";
    tno++; //going past <ret>
    ...
    tno++;//past the ;
    cout<<"exiting: Return "<<Token[tno].data<<endl;
...
void compileStatements(){
    ...
    cout<<"exiting: Statements "<<Token[tno].data<<endl;

    tno++;
}

void compileSubroutineBody(){
    cout<<"entered SubroutineBody\n";
    ...
    compileStatements();    
    tno++; //past }
    cout<<"exiting: SubroutineBody "<<Token[tno].data<<endl;
    tno++; //past </subroutineBody>
}

void compileClassVarDec(){
    cout<<"entered ClassVarDec\n";
    ...
    while(Token[tno].data == ","){
        ...
        table[variable] = bar;
        //cout<<table[variable].name<<" "<<table[variable].number<<endl;
        cout<<"exiting: classVarDec "<<Token[tno].data<<endl;
        tno++;
    }
    //currently reading a ;
    tno++; //at </classVarDec>
...
void compileSubroutineDec(){
    ...
    cout<<"exiting: SubroutineDec "<<Token[tno].data<<endl;
    tno++;
}
