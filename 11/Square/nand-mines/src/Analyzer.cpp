...
/*************************
 * CHANGE THE isOP term to include <&amp>
 * 
 * ***********************/
...
void compileReturn(){
    ...
    tno++;
    gout.close();
    //cout<<tno<<" "<<Token[tno].data<<" inside return func\n";
    if(Token[tno].data == "(" || Token[tno].type == "stringConstant" || Token[tno].type == "integerConstant" || Token[tno].type == "identifier" || Token[tno].data == "this" ||isUnary() || Token[tno].data =="null"){
        //cout<<"compileExp was called\n";
        compileExpression();
    }
    ...
    if(Token[tno].data != ";"){
            ofstream fout("err.txt");
            fout<<"Syntax Error: ; expected\n";
            fout<<Token[tno].data<<"was found instead\n";
            fout<<Token[tno-1].data<<"was the previous token\n";
...
int main(int argc, char *argv[]){
    ...
    for(int i = 1; i < argc; i++){
        filename = argv[i];
        outputfile = filename.substr(0, filename.length() - 5 ) + ".xml";
        Token.clear();
        tno = 0;
        filltokens();
        compileclass();
