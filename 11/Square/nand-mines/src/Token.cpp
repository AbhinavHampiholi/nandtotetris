...
void writexml(){
    ...
    string outputfile = filename.substr(0,filename.length()-5) + "T.xml";
    ...
    while (getline(fin, line, ' ')) {
        ...
        if(line[0]!=dq || line[line.length()-1]==dq){
...
int main(int argc, char *argv[]){
    ...
    filename = argv[i];
    pushkeyandsym();
    removecr(); // outs nocarr
    removeMulti(); //outs pass1
    removeSingle(); // outs pass2
    //DeleteEmptyLines("pass2.txt");
    removeblanklines(); //out pass3
    removeextraws(); //outs pass4
    addwhitespaces();
